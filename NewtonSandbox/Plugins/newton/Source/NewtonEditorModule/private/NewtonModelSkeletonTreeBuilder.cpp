...
#define LOCTEXT_NAMESPACE "NewtonModelSkeletonTreeBuilder"



FNewtonModelSkeletonTreeBuilderOutput::FNewtonModelSkeletonTreeBuilderOutput(TArray<TSharedPtr<class ISkeletonTreeItem>>& InItems, TArray<TSharedPtr<class ISkeletonTreeItem>>& InLinearItems)
	: Items(InItems)
	, LinearItems(InLinearItems)
{}


void FNewtonModelSkeletonTreeBuilderOutput::Add(const TSharedPtr<class ISkeletonTreeItem>& InItem, const FName& InParentName, const FName& InParentType, bool bAddToHead)
{
	Add(InItem, InParentName, TArray<FName, TInlineAllocator<1>>({ InParentType }), bAddToHead);
}

void FNewtonModelSkeletonTreeBuilderOutput::Add(const TSharedPtr<class ISkeletonTreeItem>& InItem, const FName& InParentName, TArrayView<const FName> InParentTypes, bool bAddToHead)
{
	check(0);
...
TSharedPtr<class ISkeletonTreeItem> FNewtonModelSkeletonTreeBuilderOutput::Find(const FName& InName, const FName& InType) const
{
	return Find(InName, TArray<FName, TInlineAllocator<1>>({ InType }));
}

TSharedPtr<class ISkeletonTreeItem> FNewtonModelSkeletonTreeBuilderOutput::Find(const FName& InName, TArrayView<const FName> InTypes) const
{
	check(0);
...
void FSkeletonTreeBuilder::Build(FNewtonModelSkeletonTreeBuilderOutput& Output)
{
...
void FSkeletonTreeBuilder::AddBones(FNewtonModelSkeletonTreeBuilderOutput& Output)
{
	check(0);
...
void FSkeletonTreeBuilder::AddSockets(FNewtonModelSkeletonTreeBuilderOutput& Output)
{
	check(0);
	//const USkeleton& Skeleton = EditableSkeletonPtr.Pin()->GetSkeleton();
	//
...
void FSkeletonTreeBuilder::AddAttachedAssets(FNewtonModelSkeletonTreeBuilderOutput& Output)
{
	check(0);
	//const USkeleton& Skeleton = EditableSkeletonPtr.Pin()->GetSkeleton();
	//
...
void FSkeletonTreeBuilder::AddSocketsFromData(const TArray< USkeletalMeshSocket* >& SocketArray, ESocketParentType ParentType, FNewtonModelSkeletonTreeBuilderOutput& Output)
{
	check(0);
	//const USkeleton& Skeleton = EditableSkeletonPtr.Pin()->GetSkeleton();
	//
...
void FSkeletonTreeBuilder::AddAttachedAssetContainer(const FPreviewAssetAttachContainer& AttachedObjects, FNewtonModelSkeletonTreeBuilderOutput& Output)
{
	check(0);
...
void FSkeletonTreeBuilder::AddVirtualBones(FNewtonModelSkeletonTreeBuilderOutput& Output)
{
	check(0);
...
TSharedRef<ISkeletonTreeItem> FSkeletonTreeBuilder::CreateBoneTreeItem(const FName& InBoneName)
{
	check(0);
	return MakeShareable(new FNewtonModelSkeletonTreeItem(SkeletonTreePtr.Pin().ToSharedRef()));
	//return MakeShareable(new FNewtonModelSkeletonTreeItem(InBoneName, SkeletonTreePtr.Pin().ToSharedRef()));
	//return MakeShareable(new FSkeletonTreeBoneItem(InBoneName, SkeletonTreePtr.Pin().ToSharedRef()));
}

TSharedRef<ISkeletonTreeItem> FSkeletonTreeBuilder::CreateSocketTreeItem(USkeletalMeshSocket* InSocket, ESocketParentType InParentType, bool bInIsCustomized)
{
	check(0);
	return MakeShareable(new FNewtonModelSkeletonTreeItem(SkeletonTreePtr.Pin().ToSharedRef()));
	//return MakeShareable(new FNewtonModelModelBone(FName(TEXT("xxxxx")), SkeletonTreePtr.Pin().ToSharedRef()));
	//return MakeShareable(new FSkeletonTreeSocketItem(InSocket, InParentType, bInIsCustomized, SkeletonTreePtr.Pin().ToSharedRef()));
}

TSharedRef<ISkeletonTreeItem> FSkeletonTreeBuilder::CreateAttachedAssetTreeItem(UObject* InAsset, const FName& InAttachedTo)
{
	check(0);
	return MakeShareable(new FNewtonModelSkeletonTreeItem(SkeletonTreePtr.Pin().ToSharedRef()));
	//return MakeShareable(new FNewtonModelModelBone(FName(TEXT("xxxxx")), SkeletonTreePtr.Pin().ToSharedRef()));
...
TSharedRef<ISkeletonTreeItem> FSkeletonTreeBuilder::CreateVirtualBoneTreeItem(const FName& InBoneName)
{
	check(0);
	return MakeShareable(new FNewtonModelSkeletonTreeItem(SkeletonTreePtr.Pin().ToSharedRef()));
	//return MakeShareable(new FNewtonModelModelBone(FName(TEXT("xxxxx")), SkeletonTreePtr.Pin().ToSharedRef()));
