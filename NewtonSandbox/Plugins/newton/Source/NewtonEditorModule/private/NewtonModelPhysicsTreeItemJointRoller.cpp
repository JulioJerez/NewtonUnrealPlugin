...
FNewtonModelPhysicsTreeItemJointRoller::FNewtonModelPhysicsTreeItemJointRoller(const FNewtonModelPhysicsTreeItemJointRoller& src)
	:FNewtonModelPhysicsTreeItemJoint(src)
{
}

FNewtonModelPhysicsTreeItemJointRoller::FNewtonModelPhysicsTreeItemJointRoller(TSharedPtr<FNewtonModelPhysicsTreeItem> parentNode, TObjectPtr<UNewtonLink> modelNode, FNewtonModelEditor* const editor)
	:FNewtonModelPhysicsTreeItemJoint(parentNode, modelNode, editor)
{
}

FNewtonModelPhysicsTreeItem* FNewtonModelPhysicsTreeItemJointRoller::Clone() const
{
	return new FNewtonModelPhysicsTreeItemJointRoller(*this);
}

int FNewtonModelPhysicsTreeItemJointRoller::GetFreeDof() const
{
	return 1;
}

void FNewtonModelPhysicsTreeItemJointRoller::DebugDraw(const FSceneView* const view, FViewport* const viewport, FPrimitiveDrawInterface* const pdi) const
{
	const UNewtonLinkJointRoller* const jointNode = Cast<UNewtonLinkJointRoller>(m_node);
	check(jointNode);
