...
class FNewtonModelPhysicTree : public SCompoundWidget
{
	...
	virtual ~FNewtonModelPhysicTree();
	...
	//void Construct(const FArguments& args, TSharedPtr<FNewtonModelEditor> editor);
	void Construct(const FArguments& args, const TSharedPtr<FNewtonModelEditor> editor);
	...
	//static TSharedRef<class SBoneTreeMenu> CreateVirtualBoneMenu(FNewtonModelPhysicTree* InSkeletonTree);
	//
	...
	//static void CreateMenuForBoneReduction(FMenuBuilder& MenuBuilder, FNewtonModelPhysicTree* SkeletonTree, int32 LODIndex, bool bIncludeSelected);
	//
